...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : '/cms/resourcer/samsoncms_input_wysiwyg_application?p=www/img/nicEditorIcons.gif',
...
var nicEditor = bkClass.extend({
	...
	checkReplace : function(e) {
		...
		if(r) {
			if( r.removeInstance ) r.removeInstance(e);
			if( r.removePanel ) r.removePanel();
		}
		return e;
	},
...
var nicEditorInstance = bkClass.extend({
	...
	construct : function(e,options,nicEditor) {
		...
		newX = 200 || e.clientWidth || parseInt(e.getStyle('width'));
		...
		if(isTextarea || this.options.hasPanel) {
			...
			this.editorContain = new bkElement('DIV').addClass('inner').setStyle(s).appendBefore(e);
...
var nicImageOptions = {
	buttons : {
		'image' : {name : 'Add Image', type : 'nicImageButton', tags : ['IMG'], id:'btnUpload'}
	}
...
var nicUploadButton = nicEditorAdvancedButton.extend({	
	...
	addForm : function() {
		...
		myDoc.write('<div style="font-size: 14px; font-weight: bold; padding-top: 5px;">Загрузка изображения</div>');
		myDoc.write('<input name="file" type="file" style="margin-top: 10px;" />');
		myDoc.write('</form>');
